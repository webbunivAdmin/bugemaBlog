...
import { useWriterPosts, type WriterPosts } from "@/lib/hooks/use-posts"

export default function WriterPostsTable() {
  const [statusFilter, setStatusFilter] = React.useState<string>("")
  const { mutate: deletePost } = useDeletePost()
  const user = useAuthStore((state) => state.user)
    const { data: posts, isLoading, error } = useWriterPosts(user?._id ?? "")

  const columns: ColumnDef<WriterPosts>[] = [
    {
      accessorKey: "title",
      header: ({ column }) => {
        return (
